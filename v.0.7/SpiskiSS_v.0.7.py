...
class OsnWindow(wx.Frame):
    ...
    def __init__(self, parent, spisoksov, WinPos = wx.DefaultPosition, DateUser = "01.01.2021", HSize = False, DocDir = os.getcwd(), ColouredId = "0"):
        wx.Frame.__init__(
...
#=========================================
    def OpenPanel(self):
        #form OsnPanel
        self.panel = OsnPanel(self, self.spisok, self.Date, self.FactSpisok, self.DocDir, self.ColouredId)
        try:
...
#class OsnPanel(wx.Panel):
    ...
    def __init__(self, Parent, spisok, date, FactList, DocDir, ColouredId):
        ...
        # Добавляем слушателя
        self.PanelId = "MainFrame#" + str(self.GetId())
        pub.subscribe(self.UpdateDisplay, self.PanelId)
        print("for updatedispl my id = " + self.PanelId)
        self.StartRenewThread()

        global Thread, MonitOpen
        if MonitOpen == True:
            Thread.RenewPub(self.PanelId)
    ...
    def StartRenewThread(self):
        global TimeRenew
        self.RenewThread = RenewThread(self.PanelId)
        ...
        for i in range (0, len(self.Labels)):
            if self.clickedBtn == self.Labels[i]:
                ...
                Thread = MonitThread(
                    ...
                    DocDir = self.DocDir, Pub = self.PanelId)
    ...
    #========================================
    def OpenNoteFrame(self, evt):
        print("OpenNoteFrame with btn = " + self.clickedBtn)
    ...
    def RenewWindow(self, date, stopCommand = False):
        ...
        try:
            ...
            HSize = self.frame.GetClientSize()[0]
            ...
            global WinOsn
            WinOsn = OsnWindow(None, spisoksov, WinPos = self.NewWinPos, DateUser = date, HSize = HSize, DocDir = self.DocDir, ColouredId = self.ColouredId)
            ToLog("RenewWindow finished successfully wth ColouredId = " + str(self.ColouredId))
    ...
    def UpdateDisplay(self, mess):
        ...
        try:
            if mess == "I was closed":
                ...
                self.DoRefresh(stopCommand = True)
