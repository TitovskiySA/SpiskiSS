...
class OsnWindow(wx.Frame):
    ...
    def __init__(self, parent, spisoksov, paths, WinPos = wx.DefaultPosition, DateUser = "01.01.2021", DocDir = os.getcwd(), ColouredId = "0"):
        wx.Frame.__init__(
...
#=========================================
    ...
    def OpenPanel(self):
        ...
        #form OsnPanel
        self.panel = OsnPanel(self, self.spisok, self.Date, self.DocDir, self.ColouredId)
...
#=========================================
    def RefreshPanel(self, date, colouredId):
        global spisoksov, Region, TimeRenew
        try:
            self.RenewThread.Stop()
            self.RenewThread.join()
            if len(self.panel.spisok[0]) > 0:
                prevSize = (self.GetSize()[1]-200)//len(self.panel.spisok[0])
            else:
                prevSize = (self.GetSize()[1]-200)
    
            if self.Date == date:
                if Region == 0:
                    oldList = [f"{spisoksov[:][4][i]} {spisoksov[:][3][i]}, приглашаемых: {len(spisoksov[:][6][i])} ({''', '''.join(spisoksov[:][6][i])})"  for i in range (0, len(spisoksov[:][0]))]
                else:
                    oldList = [f"{spisoksov[:][4][i]} {spisoksov[:][3][i]}" for i in range (0, len(spisoksov[:][0]))]

            spisoksov = spisoksetka(date, str(Region))
            temp = []
            for i in range (0, len(spisoksov[6])):
                if spisoksov[6][i] == ["None"]:
                    temp.append([])
                else:
                    temp.append(spisoksov[6][i][:])
            spisoksov[6] = temp[:]
        
            if self.Date == date:
                if Region == 0:
                    newList = [f"{spisoksov[:][4][i]} {spisoksov[:][3][i]}, приглашаемых: {len(spisoksov[:][6][i])} ({''', '''.join(spisoksov[:][6][i])})"  for i in range (0, len(spisoksov[:][0]))]
                else:
                    newList = [f"{spisoksov[:][4][i]} {spisoksov[:][3][i]}" for i in range (0, len(spisoksov[:][0]))]

                if oldList != newList:
                    ToLog("Edits founded")
        
                    if self.EditFrame:
                        self.EditFrame.panel.Text.AppendText(self.FindEdits(oldList, newList))
                        ToLog(self.FindEdits(oldList, newList))
                        
                    sound = SoundThread(self.DocDir + "\\Sounds\\sound2.wav")
                ...
                self.EditFrame.panel.Text.AppendText(
                    f"\nНачало мониторинга изменений в сетке совещаний на {date} в {str(datetime.today()).split()[1][0:8]}, время обновления {TimeRenew} секунд")
                self.EditFrame.panel.SetDate(date)
              
            self.sizer.Detach(self.panel)
            self.panel.Destroy()
            self.panel = OsnPanel(self, spisoksov, date, self.DocDir, colouredId)
            self.sizer.Add(self.panel, 1, wx.EXPAND, 0)
            self.Layout()
            self.SetSize((self.GetSize()[0], prevSize*len(self.panel.spisok[0])+200))
            self.StartRenewThread()
            self.Date = date
            ToLog(f"RefreshPanel Suzzessfully with date = {date}")
        except Exception as Err:
            ToLog(f"Error in RefreshPanel, error code = {Err}")
            #raise Exception
...
#==================
    ...
    def StartRenewThread(self):
        global TimeRenew
        self.RenewThread = RenewThread(self.pub)
        self.RenewThread.setDaemon(True)
        self.RenewThread.start()

        #ToLog("StartRenewThread function finished successfully")

#==================
    @ExceptDecorator
    def OnEditBtn(self):
        if not self.EditFrame:
            self.EditFrame = EditFrame(self, self.panel.NowDate.GetValue(), LoadColours(self.DocDir))
            ToLog("Opened EditFrame on date = " + str(self.panel.NowDate.GetValue()))
            sound = SoundThread(self.DocDir + "\\Sounds\\sound2.wav")
            sound.setDaemon(True)
            sound.start()
        else:
            dlg = wx.MessageDialog(
                self, "Окно мониторинга на выбранную дату уже открыто. Вы уверены, что хотите закрыть его?", " ", wx.YES_NO)
            dlg.SetYesNoLabels("&Да", "&Нет")
            if dlg.ShowModal() == wx.ID_YES:
                self.EditFrame.Destroy()
                ToLog("Closed EditFrame on date = " + str(self.panel.NowDate.GetValue()))
#========================
    def FindEdits(self, old, new):
        added = ""
        removed = ""
        preview = ""
        if set(old) == set(new):
            ToLog(
                "Something strange, sets are equal but lists are not\n" +
                "oldlist = " + str(old) + "\nnewlist = " + str(new))
        else:
            preview = "\n\n" + str(datetime.today()).split()[1][0:8] + " Обнаружены изменения по списку совещаний:\n"
            if len(list(set(new)-set(old))) > 0:
                added = "\tДобавлены совещания:\n\t\t" + "\n\t\t".join(list(set(new)-set(old))) + "\n"
            if len(list(set(old)-set(new))) > 0:
                removed = "\tУдалены совещания:\n\t\t" + "\n\t\t".join(list(set(old)-set(new)))
    ...
    def UpdateDisplay(self, mess):
        ...
        try:
            if mess == "I was closed":
                for btn in self.panel.Btn:
                    btn.SetBackgroundColour(wx.Colour(self.panel.colour[1]))
                self.panel.RunningId = False
                self.panel.StopThread()
                self.panel.ColouredId = "1"
                self.colouredId = "1"

            elif mess == "RenewNow":
                self.RefreshPanel(self.panel.NowDate.GetValue(), self.panel.ColouredId)
                
            if isinstance(mess, list):
                if mess[0] == "MyId":
                    self.panel.SetColour(mess[1])
        except Exception as Err:
            ToLog("Error in UpdateDisplay in MainFrame, Error code = " + str(Err))
            #raise Exception
...
#class OsnPanel(wx.Panel):
    ...
    def __init__(self, Parent, spisok, date, DocDir, ColouredId):
        ...
        self.DocDir = DocDir
        #print("spisoksov")
        #for sp in self.spisoksov:
        #    print(str(sp))

        global Region
        #countting users if it's Yprava
        if Region == 0:
            self.FactList = self.CountFact()
        else:
            self.FactList = False
        ...
        self.CommonVbox = wx.FlexGridSizer(rows = 5, cols = 1, hgap = 6, vgap = 6)
        ...
        LicVbox = wx.FlexGridSizer(rows = 1, cols = 2, hgap = 6, vgap = 6)
        LicVbox.AddGrowableCol(0, 1)
        ...
        MeetingVbox = wx.FlexGridSizer(rows = len(spisoksov[5]), cols = 1, hgap = 6, vgap = 6)
        #MeetingVbox = wx.FlexGridSizer(rows = len(spisoksov[5]), cols = 2, hgap = 6, vgap = 6)
        MeetingVbox.AddGrowableCol(0, 1)
        for row in range (0, len(spisoksov[5])):
            MeetingVbox.AddGrowableRow(row, 0)
        ...
        while i < len(self.spisoksov[5]):
            ...
            MeetingVbox.Add(self.NameButton, -1, wx.EXPAND|wx.LEFT|wx.RIGHT, 4)

            #self.NoteButton = wx.Button(self, wx.ID_ANY, "Note")
            #self.NoteButton.Bind(wx.EVT_BUTTON, self.OnNoteBtn)
            #self.NoteButton.SetBackgroundColour(wx.Colour(self.colour[1]))
            #self.NoteId.append((self.NoteButton.GetId(), self.spisoksov[2][i]))
            #self.Note.append(self.NoteButton)
            #MeetingVbox.Add(self.NoteButton, -1, wx.EXPAND|wx.LEFT|wx.RIGHT, 4)
            
            if self.spisoksov[2][i] == self.ColouredId:
                self.NameButton.SetBackgroundColour(wx.Colour(self.colour[2]))   
            i = i + 1

        self.CommonVbox.Add(MeetingVbox, -1, wx.EXPAND|wx.ALL, 4)
        ...
        LicVbox.Add(self.buttonlic, -1, wx.EXPAND|wx.ALL, 4)

        #adding edit Frame 
        Im = wx.Image(os.getcwd() + "\\images\\EditFrame.png").ConvertToBitmap()
        self.EditFrameBtn = wx.BitmapButton(self, wx.ID_ANY, ScaleBitmap(Im, MoveSize))
        self.EditFrameBtn.Bind(wx.EVT_BUTTON, self.OnEditBtn)
        self.EditFrameBtn.SetToolTip(f"Окно изменений на {self.Date}")
        LicVbox.Add(self.EditFrameBtn, -1, wx.ALIGN_CENTRE|wx.ALL, 4)
        
        self.CommonVbox.Add(LicVbox, -1, wx.EXPAND|wx.ALL, 4)
        ...
        #self.AddNoteMenu = wx.NewIdRef(count = 1)

        global Thread, MonitOpen
...
#================================================
    ...
    def CountLabels(self):
        ...
        if self.FactList != False:
            for k in range (0, len(self.spisoksov[3])):
                #print("Working with label = " + self.spisoksov[5][k])
                self.Labels.append(
                    self.spisoksov[4][k] + "  " + self.spisoksov[3][k] + "  (" +
    ...
    def RClick(self, evt):
        ...
        try:
            ...
            #menu.Append(self.AddNoteMenu, "Добавить заметку")
            self.Id = evt.GetId()
            self.Bind(wx.EVT_MENU, self.DoStartMonit, id = self.MonitMenu)
            #self.Bind(wx.EVT_MENU, self.OpenNoteFrame, id = self.AddNoteMenu)
        ...
        for i in range (0, len(self.Labels)):
            if self.clickedBtn == self.Labels[i]:
                ...
                self.MonitFrame = MonitFrame(
                    ...
                    DocDir = self.DocDir,
                    Pub = "MainFrame#" + str(self.GetId()))
                self.ColouredId = self.spisoksov[2][i]

                #starting thread of monitoring
                global Thread
                Thread = MonitThread(
                    [self.spisoksov[2][i], self.Date, self.spisoksov[4][i], self.spisoksov[3][i]],
                    DocDir = self.DocDir, Pub = self.frame.pub)
    ...
    #===========================================================================
    def RenewWindow(self, date, stopCommand = False):
        try:
            self.frame.RefreshPanel(date, self.ColouredId)
    ...
    #========================
    def OnEditBtn(self, evt):
        self.frame.OnEditBtn()
        
    #===========================================================================   
    # Действия при нажатой кнопке
    #def OnNoteBtn(self, event):
    #    IdNote = event.GetEventObject().GetId()
    #    for note in self.NoteId:
    #        if IdNote == note[0]:
    #            print (f"Pressed NoteBtn with id {note}")
    ...
    def DoRefresh(self, stopCommand = False):
        self.RenewWindow(self.Date, stopCommand)
    ...
    def StopThread(self):
        ...
        if Thread:
            if Thread.is_alive() == True:
                Thread.Stop = True
                ToLog("StopThread function finished!!!")
...
class MonitFrame(wx.Frame):
    def __init__(self, Label, Id, DocDir = os.getcwd(), Pub = "MainFrame"):
        self.Pub = Pub
        self.ID = Id
        wx.Frame.__init__(
            self, None, -1, Label)
        ...
        self.panel = MonitPanel(self, Id = self.ID, DocDir = DocDir, Pub = Pub)
        self.Center()
        self.Show(True)
...
class MonitPanel(wx.Panel):
    ...
    def __init__(self, parent, Id, DocDir = os.getcwd(), Pub = "Error"):
    ...
    def OnCloseWindow(self, evt):
        #try:
        #    wx.CallAfter(pub.sendMessage, self.Pub, mess = "I was closed")
...
class MonitThread(threading.Thread):
    ...
    def __init__(self, spisok, DocDir = os.getcwd(), Pub = "Error"):
        super().__init__()
        #import parameters
        #self.parent = parent
        self.Pub = Pub
...
#============================================================================
    def run(self):
        ...
        try:
            ...
            ToLog("Error sending closing mess, Error code = " + str(Err))
...
#========================
    def RenewPub(self, pub):
        ToLog("Renewed pub in Monit Thread to " + pub)
        self.Pub = pub
...
class EditFrame(wx.Frame):
    def __init__(self, parent, date, colours):
        wx.Frame.__init__(self, parent, -1, f"Окно изменений по сетке  совещаний")
        self.SetIcon(wx.Icon(os.getcwd() + "\\images\\EditFrame.png"))

        self.SetMinSize((600, 200))
        Sizer = wx.FlexGridSizer(rows = 1, cols = 1, hgap = 6, vgap = 6)
        Sizer.AddGrowableCol(0, 0)
        Sizer.AddGrowableRow(0, 0)
        self.panel = EditPanel(self, date, colours)
...
class EditPanel(wx.Panel):
    def __init__(self, parent, date, colours):
        ...
        self.SetBackgroundColour(wx.Colour(colours[0]))
        self.CommonVbox = wx.FlexGridSizer(rows = 2, cols = 1, hgap = 6, vgap = 6)
        self.CommonVbox.AddGrowableCol(0, 1)
        self.CommonVbox.AddGrowableRow(0, 0)
        ...
        self.Text = wx.TextCtrl(self, wx.ID_ANY, f"Начало мониторинга изменений в сетке совещаний на {date} в {str(datetime.today()).split()[1][0:8]}, время обновления {TimeRenew} секунд", style = wx.TE_MULTILINE | wx.TE_READONLY)
        self.Text.SetFont(wx.Font(10, wx.SWISS, wx.NORMAL, wx.NORMAL))
        self.Text.SetBackgroundColour(wx.Colour(colours[1]))
        Sizer.Add(self.Text, -1, wx.EXPAND|wx.ALL, 4)
        
        Ok = wx.Button(self, wx.ID_ANY, "Очистить окно")
        Ok.Bind(wx.EVT_BUTTON, self.OnOk)
        Ok.SetBackgroundColour(wx.Colour(colours[1]))
        Ok.SetSize((150, 40))
        Sizer.Add(Ok, -1, wx.EXPAND|wx.ALL, 4)
    ...
    def OnOk(self, evt):
        global TimeRenew
        self.Text.SetValue(f"Начало мониторинга изменений в сетке совещаний на {self.date} в {str(datetime.today()).split()[1][0:8]}, время обновления {TimeRenew} секунд")

    def SetDate(self, date):
        self.date = date
        print("Date renewed = " + date)
...
class RenewThread(threading.Thread):
    ...
    def run(self):
        global TimeRenew
        #ToLog("RenewThreadStarted with RenewTime = " + str(TimeRenew))
        try:
            startTime = time.time()
            while True:
                if self.stop == True:
                    ...
                    break
                if time.time() - startTime < TimeRenew:
                    #print("Too early for renew, last time = " + str(time.time() - startTime))
                    time.sleep(2)
                else:
                    ToLog(f"Frame was Renewed because of time Renew = {TimeRenew} elapsed!!!")
                    wx.CallAfter(
                        pub.sendMessage, self.Pub,
                        mess = "RenewNow")
                    break
            ...
            ToLog("Error in RenewThread, Error code = " + str(Err))
            #raise Exception
...
def AuthDlg(DocDir, Paths):
    try:
        ...
        if Region != 999:
            OpenMainFrame(DocDir, Paths)

    except Exception as Err:
        ToLog("Error in AthDlg, Error code = " + str(Err))
        #raise Exception

def OpenMainFrame(DocDir, Paths):
    try:
        global Region
        ...
        global WinOsn
        WinOsn = OsnWindow(None, spisoksov, paths = Paths, DateUser = todaydate, DocDir = DocDir)
...
def LoadColours(DocDir):
    global TimeRefresh, TimeRenew
    try:
        with open(DocDir + "\\Based\\" + "colours_0.8.txt", "r") as file:
        ...
        for word in info:
            if len(word.strip()) > 0:
                ...
                if word.find("colourback=") != -1:
                    ...
                    if TimeRefresh < 5:
                        TimeRefresh = 5
                elif word.find("#timerenew=") != -1:
                    TimeRenew = int(word.split()[1][:])
                    if TimeRenew < 30:
                        TimeRenew = 30
                
        #print("coloursafter func = " + str(colours) + ", time Refresh = " + str(TimeRefresh))
        ToLog("Loaded colours " + str(colours) + f", time monit meeting = {TimeRefresh}, time renew osn frame = {TimeRenew}")
        #Загружены цвета для приложения = " + str(colours))
        #ToLog("Загружено время обновления мониторинга = " + str(TimeRefresh))
        #ToLog("Загружено время обновления основного окна = " + str(TimeRenew))
...
def FindMyDir(nameDir, subDirs = None):
    try:
        ...
        if "sound2.wav" not in os.listdir(DocDir + "\\Sounds"):
            CopyFile(os.getcwd() + "\\Sounds\\sound2.wav", DocDir + "\\Sounds\\sound2.wav")
        if "colours_0.8.txt" not in os.listdir(DocDir + "\\Based"):
            CopyFile(os.getcwd() + "\\Based\\colours_0.8.txt", DocDir + "\\Based\\colours_0.8.txt")
